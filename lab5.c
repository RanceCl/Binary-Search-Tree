...
void build_optimal(Tree *T, int levels);
void build_random(Tree *T, int nodes);
void build_poor(Tree *T, int nodes);
int  build_avl_worst(Tree *T, int levels);
int  build_avl_fib(Tree *T, int levels);
...
int main(int argc, char **argv)
{
	...
	const int del[] = {100,85,125,25,50}; 
        unitDriver(ins, sizeof ins / sizeof(int),
		   del, sizeof del / sizeof(int));
    }
    if (UnitNumber == 4)
    ...
    {
        // example with poor shape that improves wtih balancing 
        const int ins[] = {13, 5, 16, 3, 10, 18, 1, 7, 12, 6};
	const int del[] = {0};
        unitDriver(ins, sizeof ins/sizeof(int), del, sizeof del/sizeof(int));
    }
    ...
    //const int del_keys[] = {100}; 


    // Equilibrium error
    const int ins_keys[] = {32, 14, 54, 10, 21, 39, 59, 6, 11, 18, 25, 36, 46, 58, 64, 2, 9, 12, 16, 20, 23, 28, 33, 37, 41, 49, 62, 4, 17, 19, 22, 27, 30, 38, 53};
    //const int del_keys[] = {26}; 
    const int del_keys[] = {59}; 

/*
Trial 190, Key 59 removed
Trial 191, Key 57 added
Trial 192, Key 41 Remove
Rightnode: 49
Rotate right
*/

        unitDriver(ins_keys, sizeof ins_keys/sizeof(int), del_keys, sizeof del_keys/sizeof(int));
    }
 ...
 */
void build_one_node(Tree *T, int key)
{
 ...
 */
void build_poor(Tree *T, int levels)
{
    int n, start, inc, i, j;
    int low, high;
    n = pow(2, levels);
 ...
 */
void build_avl_fib_rec(Tree *T, int levels, int offset)
{
    if (levels == 0)
