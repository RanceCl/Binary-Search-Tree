...
//int AVLInsert (Tree *T, TreeNode *TN, TreeNode *NN);
...
//	assert(BalanceOption == balanceoptions;
...
void recursiveTreeDelete(TreeNode *N)
{
	...
	if(N->MidLink != NULL)
	{
		recursiveTreeDelete(N->MidLink);		//Remove data in middle leaf
		N->MidLink = NULL;
	}
	if(N->RightLink != NULL)
	{
...
int tree_insert (Tree *T, KeyType Key, DataType DataPointer)
{
	...
	NewNode->NodeItem2 = 0;
	NewNode->DataPtr2 = NULL;
	NewNode->MidLink = NULL;
...
TreeNode *AVLSearch (Tree *T, TreeNode *N, TreeNode *Holder, int Operation, int *KR)
{
	...
	{
		//Double left rotate
		if(Holder->NodeItem < N->RightLink->NodeItem) N->RightLink = RotateRight(N->RightLink);
